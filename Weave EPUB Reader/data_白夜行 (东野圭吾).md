---
format: "weave-epub-bookmarks/v3"
weave_epub_bookmark_file: true
stableKey: "epub-book-17to268"
bookId: "epub-book-17to268"
bookPath: "不妨来看看看书/白夜行 (东野圭吾).epub"
displayTitle: "白夜行 (东野圭吾)"
bookTitle: "白夜行 (东野圭吾)"
bookAuthor: "(日)东野圭吾"
bookLanguage: "zh"
publisher: "南海出版社"
publishDate: "2015-03-14"
coverPath: "/"
wordCount: 189148
chapterCount: 34
reading-progress: 2
reading-status: "reading"
reading-total-minutes: 0
reading-wpm: 0
highlight-count: 0
excerpt-note-count: 0
bookmark-count: 0
last-read-at: 1785146824174
readingState:
  currentPosition:
    chapterIndex: 4
    cfi: "epubcfi(/6/10!/4,/2,/14/1:39)"
    percent: 2
  readingStats:
    totalReadTime: 11033
    lastReadTime: 1785146824174
    createdTime: 0
    paceSampleCount: 0
    paceSampleWords: 0
bookmarks: []
analytics:
  updatedAt: 1785146822424
  highlightsByColor: {}
  commentCount: 0
  concealedCount: 0
  topChaptersByHighlights: []
user:
  tags: []
  rating: null
  priority: ""
  notes: ""
updatedAt: 1785302410051
---

> 📎 本页由 Weave EPUB 自动维护。YAML 中 `readingState`、`bookmarks` 请勿手改；`user` 与「我的标注」可自由编辑。

# 📚 白夜行 (东野圭吾)

---

## 📖 书籍信息

| | |
| :-- | :-- |
| 封面 | ![[/|200]] |
| 书名 | 白夜行 (东野圭吾) |
| 作者 | (日)东野圭吾 |
| 出版社 | 南海出版社 |
| 出版时间 | 2015-03-14 |
| 字数 | 18.9 万 |
| 章节 | 34 章 |
| 语言 | zh |
| 书籍文件 | [[不妨来看看看书/白夜行 (东野圭吾).epub|📂 白夜行 (东野圭吾)]] |

---

## 📊 阅读进度

| 项目 | 值 |
| :-- | :-- |
| 状态 | 🟢 阅读中 |
| 进度 | 2% |
| 当前章节 | 第 5 章 |
| 累计阅读 | 0 分钟 |
| 阅读速度 | — |
| 最近阅读 | 2026-07-27 18:07 |

> [!tip] 📍 继续阅读
> 当前停在 **第 5 章**（约 2%）。在阅读器中打开本书即可续读。

---

## ✨ 标注统计

| 项目 | 数量 |
| :-- | --: |
| 🖍️ 高亮 | 0 |
| 📝 关联笔记 | 0 |
| 🔖 书签 | 0 |
| 💬 批注 | 0 |

---

## 🔖 书签

暂无书签

---

## 💡 最近摘录

暂无摘录

---

## 📈 阅读分析

暂无高亮数据

---

## 🔗 关联笔记

暂无关联笔记

---

## ✏️ 我的标注

| 项目 | 内容 |
| :-- | :-- |
| 🏷️ 标签 |  |
| ⭐ 评分 |  |
| 🚩 优先级 |  |
| 📋 备注 |  |

> [!quote] ✏️ 可编辑区
> 在上方表格或此处自由书写；插件不会覆盖本节。

---

*weave-epub-bookmarks/v3 · Weave EPUB 自动维护*